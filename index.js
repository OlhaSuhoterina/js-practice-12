...
function convertMapToObject(map) {
  ...
  const N = Object.fromEntries(iter);
...
function setValue(dictionary, key, value) {
  let N = dictionary.has(key);
  //
  if ((N = true)) {
    dictionary.set(key, value);
  } else {
    dictionary.set(`${value}`, value);
  }
  return dictionary;
